---
title: "Microarray Data - H.pylori infection and IFN treatment - GSEA analysis on DGE results"
...
```{r}
gsea_gmt_folder = file.path(root_folder,"Data/External/GeneSets/MSigDB_v6.2")
...
beta_cat_genes = as.data.frame(read_excel(file.path(root_folder, "Data/External/GeneSets/Wnt target H2014 list.xls"), sheet = 1, col_names = F))
...
tmp = read.table(file.path(root_folder, "Data/External/GeneSets/Supp_Table_S3_GE_combined_SC_Signature.txt"), sep="\t", header=T, stringsAsFactors = F)
custom_gene_sets_mouse[["Lgr5_SC_Signature_Munoz"]] = unique(tmp$GeneSymbol)

tmp = as.data.frame(read_excel(file.path(root_folder, "Data/External/GeneSets/antral lgr5 signature from barker 2010.xls"),sheet=1))
